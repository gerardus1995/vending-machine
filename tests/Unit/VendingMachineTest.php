...
final class VendingMachineTest extends TestCase
{
    ...
    public function testSelectProductSuccessExactChange(): void
    {
        ...
        // Check state changes: product stock decreased, coin inventory updated
        $this->assertEquals(0, $this->vendingMachine->getProductStock()['water']); // Assuming default stock is 1
    ...
    public function testSelectProductSuccessWithChange(): void
    {
        ...
        $this->vendingMachine->service(
            $this->vendingMachine->getProductStock(), // current product stock (from default repo)
        ...
        $this->assertEquals(2, $this->vendingMachine->getCoinInventory()[100] ?? 0); // inserted 2, returned 0 of 100c
        $this->assertEquals(8, $this->vendingMachine->getCoinInventory()[25] ?? 0);   // started with 10, got 2 back from transaction, used 4 for change = 8
    ...
    public function testSelectProductUnableToMakeChange(): void
    {
        ...
        // First, service the machine with empty coin inventory
        $this->vendingMachine->service(
            $this->vendingMachine->getProductStock(), // current product stock (from default repo)
